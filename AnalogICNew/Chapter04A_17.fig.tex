...
\draw (xL|-yC) node[left] {$v_{IN}$} to[short,o-*] (M1.G|-yC);
\draw (M1.G) -- (M2.G);

\draw (M1.G) to[short,*-] (M1.G|-M1.D) to[C,l=$C_{gd1}$,*-*] (M1.D);
\draw (M2.G) to[short,*-] (M2.G|-M2.S) to[C,l=$C_{gs1}$,-*] (M2.S);
\draw (M1.G) to[short,*-] (M1.G|-M1.S) to[C,l_=\raisebox{0.5cm}{$C_{gs1}$},-*] (M1.S);
\draw (M2.G) to[short,*-] (M2.G|-M2.D) to[C,l_=\raisebox{0.5cm}{$C_{gd2}$},*-*] (M2.D);
