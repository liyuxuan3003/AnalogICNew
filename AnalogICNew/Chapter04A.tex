...
推挽反相放大器的大信号特性如\xref{fig:推挽反相放大器的大信号特性}所示。由于推挽放大器中$M_1,M_2$均与$v_{IN}$有关，故其负载图较为复杂，如\xref{fig:负载图--推挽反相}所示，对于每一个$v_{IN}$的取值，现在$M_1,M_2$都各有一条曲线。
...
我们最终期望的形式是
\begin{Equation}
    A_v(s)=A_v\frac{1-s/\omega_{z1}}{(1-s/\omega_{p1})(1-s/\omega_{p2})}
\end{Equation}
