...
我们现在来解决另外一个大信号问题：输出电压的摆幅是多少？要明确的是，输出电压的摆幅并非是指$M_1,M_2$均位于饱和区时输出$v_{OUT}$的变化范围，而是指：输入$v_{IN}$从最小变化至最大，输出$v_{OUT}$的变化范围是多少？在理想状态下，输出摆幅应当是轨到轨的，换言之，应有$V_{OUT,\max}=V_{DD}$和$V_{OUT,\min}=0$。但是从\xref{fig:电压转移特性--二极管反相}中我们可以看出，这里两者均不满足。
...
\begin{BoxFormula}[二极管负载反相放大器--最大输出电压]
...
\begin{BoxFormula}[电流源负载反相放大器--最大输出电压]
...
\end{Equation}
整理如下
\begin{BoxFormula}[电流源负载反相放大器--最小输出电压]
...
\begin{BoxFormula}[推挽反相放大器--最大输出电压]
    推挽反相放大器中，输出电压的最大值为
    \begin{Equation}
        V_{OUT,\max}=V_{DD}
    \end{Equation}
\end{BoxFormula}
\begin{BoxFormula}[推挽反相放大器--最小输出电压]
    推挽反相放大器中，输出电压的最小值为
    \begin{Equation}
        V_{OUT,\min}=0
    \end{Equation}
...
\subsection{反相器的噪声特性}
在本小节，我们来分析反相放大器的噪声特性。我们知道，MOS的噪声特性可以由一个串联在其栅端的噪声源来模拟，\xref{fig:反相放大器的噪声}展现了考虑噪声源后三种反相放大器的电路结构。
