...
\begin{Gather}
    i_{out}=g_{m4}v_{1}+g_{bs4}(-v_2)+g_{ds4}(v_{out}-v_2)\xlabelpeq{5} \\
...
\begin{Equation}&[11]
    \qquad\qquad
    i_{out}\qty[1+g_{m1}g_{m4}g_{ds1}^{-1}(g_{m2}+g_{ds2})^{-1}+(g_{bs4}+g_{ds4})(g_{m2}+g_{ds2})^{-1}]=g_{ds4}v_{out}
    \qquad\qquad
\end{Equation}
这就得到
\begin{Equation}&[12]
    \qquad
    R_{out}=\frac{v_{out}}{i_{out}}=g_{ds4}^{-1}\qty[1+g_{m1}g_{m4}g_{ds1}^{-1}(g_{m2}+g_{ds2})^{-1}+(g_{bs4}+g_{ds4})(g_{m2}+g_{ds2})^{-1}]
    \qquad
\end{Equation}
就方括号内的项做一些整理
\begin{Equation}&[13]
    R_{out}=g_{ds4}^{-1}\qty[1+(g_{m1}g_{m4}g_{ds1}^{-1}+g_{bs4}+g_{ds4})(g_{m2}+g_{ds2})^{-1}]
\end{Equation}
将结果整理如下
...
若忽略些不重要的项，这里$R_{out}$可以近似为$R_{out}=g_{m1}^{\vphantom{-1}}g_{m2}^{-1}g_{m4}^{\vphantom{-1}}g_{ds1}^{-1}g_{ds4}^{-1}$，这表明，威尔逊电流镜具有和共源共栅电流镜一样的$R_{out}=g_{m}g_{ds}^{-2}$量级的输出电阻，验证了分析\xref{fig:威尔逊电流镜--基础型的大信号特性}时的说法。
