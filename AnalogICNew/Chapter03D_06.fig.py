...
plt.plot(d["Vout"],d["VD3"],c='r',label="$v_{D3}$")
...
plt.figure(idVSAT3)
plt.plot(d["Vout"],d["VGT3"],c='g',label="$v_{GS3}-V_{T}$")
plt.plot(d["Vout"],d["VDS3"],c='purple',label="$v_{DS3}$")
...
for id in range(graphNum):
    ...
    if id in[idIV,idVD,idVSAT1,idVSAT2,idVSAT3]:
        ...
        if id in[idVD,idVSAT1,idVSAT2,idVSAT3]:
