...
\end{Equation}
现在的问题是，这里$I_D$与$I_{D0}$间是什么关系？我们或许会设想$I_D=I_{D0}$是不随温度变化的电流源，但到目前为止，我们唯一可用的可靠电流源就是\xref{subsec:PTAT电压和电流的生成}中的PTAT电流，因此
\begin{Equation}&[10]
    \frac{I_D}{I_{D0}}=\qty(\frac{T}{T_0})
\end{Equation}
为了一般化，这里引入参数$\alpha$，并且这里$\alpha=1$
\begin{Equation}&[11]
    \frac{I_D}{I_{D0}}=\qty(\frac{T}{T_0})^{\alpha}
\end{Equation}
这样一来，将\xrefpeq{11}代入\xrefpeq{9}中，得到
\begin{Equation}
    V_{D}=V_{G0}-(V_{G0}-V_{D0})\qty(\frac{T}{T_0})-V_t(\gamma-\alpha)\ln(\frac{T}{T_0})
\end{Equation}
这里的$V_D$就是所需的$V_{CTAT}$，整理如下
\begin{BoxFormula}[CTAT电压]
    CTAT电压可以表示为
    \begin{Equation}
        V_{CTAT}=V_{G0}-(V_{G0}-V_{D0})\qty(\frac{T}{T_0})-V_t(\gamma-\alpha)\ln(\frac{T}{T_0})
    \end{Equation}
\end{BoxFormula}
...
\begin{itemize}
    ...
    \item $V_{CTAT}$随温度变化是非线性的，在\xref{fig:PTAT电压和CTAT电压}中，蓝色实线和蓝色虚线分别代表考虑和不考虑$V_{CTAT}$中非线性项$-V_t(\gamma-\alpha)\ln(T/T_0)$的曲线，可以明显看出两者间的偏差。
    ...
    % \item $V_{CTAT}$随温度的变化是非线性的，这来自$-V_t(\gamma-\alpha)\ln(T/T_0)$，其中对数造成了非线性，
\end{itemize}
...
\begin{enumerate}
    \item 串联型：$I_{PTAT}'$流经电阻变回$V_{CTAT}$与$V_{PTAT}$串联电压相加，如\xref{fig:串联型}所示。
...
\begin{Equation}&[2]
    \qquad\qquad
    \dv{V_{CTAT}}{T}=-(V_{G0}-V_{D0})\qty(\frac{1}{T_0})-\frac{V_t}{T_0}(\gamma-\alpha)\ln(\frac{T}{T_0})-V_t(\gamma-\alpha)\qty(\frac{T_0}{T})\qty(\frac{1}{T_0})
    \qquad\qquad
\end{Equation}
观察\xrefpeq{1}，注意到
\begin{Equation}&[3]
    \frac{V_{CTAT}-V_{G0}}{T}=-(V_{G0}-V_{D})\qty(\frac{1}{T_0})-\frac{V_t}{T}(\gamma-\alpha)\ln(\frac{T}{T_0})
\end{Equation}
我们试着用\xrefpeq{3}代换\xrefpeq{2}的前半部分
\begin{Equation}&[4]
    \qquad\qquad
    \dv{V_{CTAT}}{T}=\frac{V_{CTAT}-V_{G0}}{T}+\qty(\frac{V_t}{T}-\frac{V_{t}}{T_0})(\gamma-\alpha)\ln(\frac{T}{T_0})-\qty(\frac{V_t}{T})(\gamma-\alpha)
    \qquad\qquad
\end{Equation}
整理如下
\begin{BoxFormula}[CTAT电压随温度的变化率]
    ...
    特别的，当$T=T_0$时
    \begin{Equation}
        \eval{\dv{V_{CTAT}}{T}}_{T=T_0}=\frac{V_{CTAT0}-V_{G0}}{T_0}-\qty(\frac{V_t}{T_0})(\gamma-\alpha)
    \end{Equation}
\end{BoxFormula}
...
\begin{Equation}
    K=-\frac{V_{CTAT0}-V_{G0}-V_t(\gamma-\alpha)}{V_{PTAT0}}
\end{Equation}
这就是我们所要求的温度无关条件，整理如下。
\begin{BoxFormula}[温度无关条件]
    温度无关条件为
    \begin{Equation}
        K=\frac{R_2}{R_1}=-\frac{V_{CTAT0}-V_{G0}-V_t(\gamma-\alpha)}{V_{PTAT0}}
    \end{Equation}
\end{BoxFormula}
...
\begin{BoxFormula}[串联型温度无关的基准的室温值]
    串联型温度无关的基准在室温下$V_{REF0}$为
    \begin{Equation}
        V_{REF0}=\frac{R_3}{R_2}\qty[V_{G0}+V_t(\gamma-\alpha)]
    \end{Equation}
\end{BoxFormula}
