...
我们现在开始构建设计流程，如\xref{tab:差分放大器的设计流程}所示
\begin{enumerate}
...
\begin{Table}[差分放大器的设计流程]{p{2cm}p{\textwidth-4cm-6\tabcolsep}>{\raggedleft\arraybackslash}p{2cm}}
    <计算步骤&计算式&结果\\>
    ...
    &$g_{ds6}=\lambda_6I_{D6}$&$\SI{4.0}{uS}$\\ \hlinelig
    02&$V_{ON4}=V_{DD}-V_{IC,\max}$&$\SI{0.50}{V}$\\
    &$W_4/L_4=2I_{D4}/K_4'V_{ON4}^2$&$\SI{8.00}{}$\\
    &$g_{m4}=2I_{D4}/V_{ON4}$&$\SI{200}{uS}$\\ \hlinelig
    03&$g_{m2}=A_v(g_{ds2}+g_{ds4})$&$\SI{450}{uS}$\\
    &$W_2/L_2=\sqrt{2I_{D2}K_2'/g_{m2}^2}$&$\SI{18.41}{}$\\
    &$V_{ON2}=2I_{D2}/g_{m2}$&$\SI{0.22}{V}$\\  \hlinelig
    04&$V_{ON5}=V_{IC,\min}-V_{ON2}-V_{T2}$&$\SI{0.08}{V}$\\
    &$W_5/L_5=2I_{D5}/K_5'V_{ON5}^2$&$\SI{300.56}{}$\\
    &$g_{m5}=2I_{D5}/V_{ON5}$&$\SI{2571}{uS}$\\ \hlinelig
    05&$V_{ON6}=V_{ON5}$&$\SI{0.08}{V}$\\
    &$W_6/L_6=W_5/L_5$&$\SI{300.56}{}$\\
    &$g_{m6}=g_{m5}$&$\SI{2571}{uS}$\\
\end{Table}

我们将设计结果整理在\xref{tab:差分放大器的设计结果}中
